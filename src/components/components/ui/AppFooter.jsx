...
const softEnvelopeShadow = {
  background: [
    'linear-gradient(0deg, rgba(3,27,18,0.35) 0%, rgba(3,27,18,0.19) 34%, rgba(3,27,18,0.08) 64%, rgba(3,27,18,0) 100%)',
    'radial-gradient(ellipse at 50% 100%, rgba(3,27,18,0.29) 0%, rgba(3,27,18,0.16) 42%, rgba(3,27,18,0) 76%)',
  ].join(', '),
};
  ...
  return (
    ...
    >
      <div className="relative h-full w-full overflow-visible">
        <div
          className="pointer-events-none fixed inset-x-0 bottom-0 z-[0] overflow-hidden border-t border-[#F5F1E8]/28 shadow-[0_-12px_23px_rgba(3,27,18,0.14),0_-3px_8px_rgba(3,27,18,0.10)]"
          style={{ height: `calc(${APP_FOOTER_HEIGHT_PX}px + env(safe-area-inset-bottom))`, ...pagePitchMowBackground }}
          aria-hidden="true"
        />
        <span
          className="pointer-events-none fixed inset-x-0 bottom-[calc(52px+env(safe-area-inset-bottom))] z-[0] h-px bg-[#F5F1E8]/42 shadow-[0_1px_0_rgba(3,27,18,0.17)]"
          aria-hidden="true"
        />
        <div
        ...
        <div
          ...
          style={{
            background: 'linear-gradient(0deg, rgba(3,27,18,0.23), rgba(3,27,18,0.08) 52%, rgba(3,27,18,0))',
            filter: 'blur(8px)',
          }}
          aria-hidden="true"
        />
        <div
          className="relative h-full w-full overflow-hidden border-t border-[#F5F1E8]/28 shadow-[0_-12px_23px_rgba(3,27,18,0.14),0_-3px_8px_rgba(3,27,18,0.10)]"
          style={pagePitchMowBackground}
        >
          <span
            className="pointer-events-none absolute inset-x-0 top-0 h-px bg-[#F5F1E8]/42 shadow-[0_1px_0_rgba(3,27,18,0.17)]"
            aria-hidden="true"
          />
          <span
            className="pointer-events-none absolute inset-x-0 top-0 h-[76%]"
            style={{
              background: 'linear-gradient(180deg, rgba(3,27,18,0.23) 0%, rgba(3,27,18,0.11) 35%, rgba(3,27,18,0.02) 100%)',
            }}
            aria-hidden="true"
          />
