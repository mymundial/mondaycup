...
export function ScreenTopBar({ children, menuOpen, onToggleMenu, onMatch, onFixtures, onGroups, onClubhouse, onTrophyCabinet, onLeaderboard, onRestart, onSignOut, canSignOut }) {
  return (
    <section
    ...
    >
      ...
      <button onClick={onToggleMenu} aria-label="Open menu" className="absolute right-3 top-1/2 z-[1001] flex h-10 w-10 -translate-y-1/2 items-center justify-center text-[#F5F1E8]">
        <HamburgerIcon />
      </button>
      {menuOpen && <MenuDropdown onClose={onToggleMenu} onMatch={onMatch} onFixtures={onFixtures} onGroups={onGroups} onClubhouse={onClubhouse} onTrophyCabinet={onTrophyCabinet} onLeaderboard={onLeaderboard} onRestart={onRestart} onSignOut={onSignOut} canSignOut={canSignOut} />}
    </section>
  );
}
