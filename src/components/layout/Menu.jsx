...
export function MenuDropdown({ onClose, onMatch, onFixtures, onGroups, onClubhouse, onTrophyCabinet, onLeaderboard, onRestart }) {
  const menu = (
    <div className="fixed inset-0 isolate flex justify-center bg-[#072D1D]/18" style={{ zIndex: 2147483000 }}>
      ...
      <div className="pointer-events-none relative z-[1] h-[100dvh] w-full max-w-[1080px] px-5 pt-3">
        <div className="pointer-events-auto absolute right-5 top-3 w-[190px] overflow-hidden rounded-[1.25rem] bg-[#F8F4EC] pb-2 text-[#0B5F35] shadow-[0_14px_30px_rgba(7,45,29,0.14)]">
          <div className="relative flex h-12 items-center justify-center bg-[#0B5F35] text-[#F5F0E6]">
          ...
          <div className="px-2 pt-1.5">
            ...
            <div className="mx-2 my-1.5 h-px bg-[#0B5F35]/10" />
...
export function ScreenTitle({ children, menuOpen, onToggleMenu, onMatch, onFixtures, onGroups, onClubhouse, onTrophyCabinet, onLeaderboard, onRestart }) {
  return <section className="relative z-[200] mb-2 flex h-[54px] shrink-0 items-center justify-center overflow-visible bg-[#F5F1E8] text-[#0B5F35] shadow-[0_2px_8px_rgba(0,0,0,0.08)]">
