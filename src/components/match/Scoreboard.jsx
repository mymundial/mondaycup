...
function PenaltyMarkers({ attempts, totalSlots = GAME.regulationPens }) {
  ...
  const displaySlots = Math.min(MAX_VISIBLE_MARKERS, Math.max(totalSlots, allMarkers.length, GAME.regulationPens));
  const visibleMarkers = allMarkers.length > displaySlots ? allMarkers.slice(-displaySlots) : allMarkers;
  ...
  return (
    <div className="inline-flex w-max max-w-none justify-center overflow-visible" style={{ gap: gapSize }}>
      {Array.from({ length: displaySlots }).map((_, idx) => {
        const value = visibleMarkers[idx];
        ...
        return <span key={idx} data-share-marker-dot="true" className={`shrink-0 rounded-full ${color}`} style={{ width: dotSize, height: dotSize, boxShadow: "0 0 2.5px rgba(247,209,23,0.13)", filter: "none" }} />;
      })}
    </div>
  );
}
    ...
    <section data-share-scoreboard="true" className={`relative mt-0 ${heightClass} shrink-0 overflow-visible bg-[#050505]`} style={heightStyle}>
      <div data-share-scoreboard-main="true" className="relative overflow-hidden border-y border-[#F5F1E8]/18 bg-[#050505] shadow-[0_2px_8px_rgba(0,0,0,0.20)]" style={{ height: `${100 - tickerPercent}%` }}>
        ...
        <div className="relative z-[1] flex h-full items-center px-[clamp(8px,3.5%,18px)] py-0">
          <div className="grid h-[88%] w-full grid-cols-[34px_minmax(0,1fr)_clamp(34px,10vw,48px)_clamp(34px,9vw,46px)_clamp(34px,10vw,48px)_minmax(0,1fr)_34px] grid-rows-[30%_45%_25%] items-center">
            {!hideStageLabel && (
              <div data-normalise-stage-label="true" className="col-start-2 col-end-7 row-start-1 flex items-center justify-center">
                <div className="inline-flex min-h-[clamp(15px,1.95vh,20px)] max-w-full items-center justify-center rounded-[0.32rem] border border-[#F5F1E8]/22 bg-[#050505] px-[clamp(5px,1.45vw,9px)] py-0 shadow-[inset_0_1px_0_rgba(245,241,232,0.08)]">
                  <div className="led-text-glow font-led flex min-h-full items-center justify-center whitespace-nowrap text-center text-[clamp(5.8px,0.95vh,10px)] font-black uppercase leading-none text-[#F7D117]" style={{ fontFamily: "IntoDotMatrix, monospace", fontWeight: 900, ...scoreboardLabelTextStyle }}>
                    <DotMatrixLabelText>{normaliseThirdPlaceCopy(stageLabel || "GROUP STAGE")}</DotMatrixLabelText>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="relative z-[1] col-start-2 row-start-3 flex h-full min-w-0 items-center justify-center overflow-visible">
            ...
            <div className="col-start-6 row-start-2 flex h-full min-w-0 items-center justify-center overflow-visible px-[clamp(8px,3vw,18px)]">
            ...
            <div className="relative z-[1] col-start-6 row-start-3 flex h-full min-w-0 items-center justify-center overflow-visible">
            ...
            <div data-share-username-slot="true" className="relative z-[3] col-start-3 col-end-6 row-start-3 flex h-full items-center justify-center px-[clamp(5px,1.5vw,10px)]">
              <div className={`${usernameEnabled ? "visible" : "invisible"} led-text-glow font-led inline-flex min-h-[clamp(15px,1.95vh,20px)] max-w-full items-center justify-center truncate whitespace-nowrap rounded-[0.32rem] border border-[#F5F1E8]/22 bg-[#050505] px-[clamp(5px,1.45vw,9px)] py-0 text-center text-[clamp(5.8px,0.95vh,10px)] font-black uppercase leading-none shadow-[inset_0_1px_0_rgba(245,241,232,0.08)]`} style={{ color: usernameToneColour, ...scoreboardLabelTextStyle }}>
                <DotMatrixLabelText>{cleanUsername || "GUEST"}</DotMatrixLabelText>
              </div>
            </div>
          </div>
        </div>
