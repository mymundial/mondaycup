...
function BracketSlot({ value, size = "sm", userTeam = null }) {
  ...
  const isUserTeam = isRealTeam(value) && value === userTeam;

  if (!isRealTeam(value)) {
    return <PlaceholderSlot value={value || "TBC"} size={size} />;
  }

  return <span className={`relative inline-flex ${sizeClass} shrink-0 overflow-hidden rounded`}>
    <Flag team={value} className="h-full w-full rounded" />
    {isUserTeam && <span className="pointer-events-none absolute inset-0 rounded border-[2px] border-[#7DAA8F] shadow-[inset_0_0_0_1px_rgba(245,240,230,0.55)]" />}
  </span>;
}
    ...
    <div className="origin-top scale-[0.86] px-0 pb-0">
      ...
      <div className="mt-5">
        <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3">
          <div><StageLabel>3RD PLACE PLAY-OFF</StageLabel><div className="mt-1.5"><BracketRow count={1} fixtures={thirdPlace} size="final" layout="vertical" userTeam={userTeam} /></div></div>
          <PodiumStack podium={podium} />
          <div><StageLabel>FINAL</StageLabel><div className="mt-1.5"><BracketRow count={1} fixtures={final} size="final" layout="vertical" userTeam={userTeam} /></div></div>
        </div>
      </div>
