...
function KnockoutBracket({ round32 = [] }) {
  const fixtures = round32.length ? round32 : buildRound32Placeholders();
  const FlagSlot = ({ team = null, seed = null, compact = false, large = false }) => {
    const value = team || seed || "TBC";
    const showFlag = value !== "TBC" && !isSeedLabel(value);
    return <div className={`flex items-center justify-center overflow-hidden rounded-[0.6rem] border border-[#0B5F35]/10 bg-[#7DAA8F] text-[#F5F0E6] ${large ? "h-9 w-14 text-[9px]" : compact ? "h-6 w-8 text-[7px]" : "h-7 w-10 text-[8px]"}`}>{showFlag ? <Flag team={value} className="h-full w-full rounded-none" /> : <span className="font-black uppercase tracking-[0.04em]">{value}</span>}</div>;
  };
  const MatchSlot = ({ match, compact = false, large = false }) => <div className="flex flex-col items-center justify-center gap-[2px]"><div className="text-[6px] font-black text-[#0B5F35]/45">M{match?.matchNo || ""}</div><div className="flex items-center justify-center gap-[2px]"><FlagSlot team={match?.home} seed={match?.homeSeed} compact={compact} large={large} /><FlagSlot team={match?.away} seed={match?.awaySeed} compact={compact} large={large} /></div></div>;
  const Row = ({ count, matches = [], compact = false, gap = "gap-1", large = false }) => <div className={`grid ${count === 8 ? "grid-cols-8" : count === 4 ? "grid-cols-4" : count === 2 ? "grid-cols-2" : "grid-cols-1"} ${gap}`}>{Array.from({ length: count }).map((_, index) => <MatchSlot key={index} match={matches[index]} compact={compact} large={large} />)}</div>;
  const StageLabel = ({ children }) => <div className="text-center text-[7px] font-black uppercase tracking-[0.12em] text-[#7DAA8F]">{children}</div>;
