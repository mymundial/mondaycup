...
export default function FeedbackModal({ open = false, promptType = "prompt1", onSubmit, onDismiss }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#020805]/72 px-4 py-6 backdrop-blur-[3px]">
      <div
        className="relative w-full max-w-[408px] overflow-hidden rounded-[1.7rem] border border-[#F5F1E8]/18 bg-[#0d6c3d] p-4 text-[#F5F1E8] shadow-[0_24px_54px_rgba(0,0,0,0.42),inset_0_1px_0_rgba(255,255,255,0.1)] ring-1 ring-[#F7D117]/12"
      ...
      >
        ...
        <div className="relative z-10 grid min-h-[48px] grid-cols-[44px_minmax(0,1fr)_44px] items-center gap-2">
          <img
            src={MONDAY_CUP_SHIELD_SRC}
            alt="Monday Cup"
            className="h-10 w-10 justify-self-start object-contain drop-shadow-[0_6px_10px_rgba(0,0,0,0.35)]"
            draggable="false"
          />

          <div className="home-copy-bold min-w-0 text-center text-[25px] uppercase leading-none tracking-[0.12em] text-[#F5F1E8]">
            RATE US
          </div>
          ...
          <button
            type="button"
            onClick={onDismiss}
            className="grid h-10 w-10 place-items-center justify-self-end text-[#F5F1E8]/92 drop-shadow-[0_2px_5px_rgba(0,0,0,0.35)] active:scale-[0.96]"
        ...
        {submitted ? (
          ...
          <div className="relative mt-3 space-y-3 text-center">
            <div className="rounded-[1.35rem] border border-[#F5F1E8]/14 bg-[#031B12]/44 px-4 pb-5 pt-6 shadow-[inset_0_1px_0_rgba(255,255,255,0.07)]">
              <div className="home-copy-bold mx-auto max-w-[315px] text-[14px] uppercase leading-none tracking-[0.13em] text-[#F5F1E8]">
                ENJOYING MONDAY CUP?
              </div>

              <div className="mt-5 flex justify-center gap-2">{[1, 2, 3, 4, 5].map((star) => <StarButton key={star} index={star} selected={star <= rating} onSelect={setRating} />)}</div>

              <div className="home-copy-regular mx-auto mt-5 max-w-[315px] text-[10px] uppercase leading-[1.55] tracking-[0.12em] text-[#F5F1E8]/74">
                YOUR FEEDBACK HELPS SHAPE FUTURE TOURNAMENTS.
              </div>
            ...
            <div className="rounded-[1.35rem] border border-[#F5F1E8]/14 bg-[#031B12]/44 p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.07)]">
